...
public class SecurityRoom : MonoBehaviour
{
    ...
    private void OnTriggerStay()
    {
        ...
        {
            if (GameManager._OpenInDoorSecurityRoom)
            {
                MapManager.MainCorridor = new Color(1, 1, 1);
                MapManager.SecurityRoom = new Color(1, 0.392f, 0.392f);
                SceneManager.LoadScene("SecurityRoom", LoadSceneMode.Single);
            }
            else
            {
                if (InventoryPlayer.ItemSearch("Ключ-карта"))
                {
                    ...
                    inventory.removeOldItemInInventory("Ключ-карта");

                    GameManager.triggerInDoorSecurityRoom(true);

                    OpenInDialog(dialog1);
                    MapManager.DoorSecurityRoom = new Color(0, 0.22f, 1);
                }
                else
                {
                    OpenInDialog(dialog);
                    MapManager.DoorSecurityRoom = new Color(1, 4.8f, 0);
                }
            }
